...
void main() {
	...
	group('Methods desciption', () {
		...
		test('TwoMethodsWidParameters class gets 2 methods and 2 parameters', () {
			...
			var methodDescriptions = hubsInspector.hubDescriptions['TwoMethodsWidParameters']
				.methods;
		...
		test('OneMethodWidDefaultValueParameters class gets 1 method and parameters with default value',
				() {
				expect(hubsInspector.hubDescriptions
					.containsKey('OneMethodWidDefaultValueParameters'),
					isTrue);
	...
	group('hubDescriptions serialization', () {
		setUp(() {
			hubsInspector.inspectCode();
		});
		test('hubDescrptor serialization gets the right names', () {
			hubsInspector.hubDescriptions.forEach((name, hubDescriptor) {
				Map json = hubDescriptor.toMap();
				expect(json['name'], name);
			});
		});

		test('methods serializing gets the right name', () {
			HubDescriptor descriptor = hubsInspector
				.hubDescriptions['OneMethodWidDefaultValueParameters'];
			Map method = descriptor.toMap()['methods'][0];
			expect(method['name'], 'm1');
		});

		test('parameters serializing gets the right name and default value', () {
			HubDescriptor descriptor = hubsInspector
				.hubDescriptions['OneMethodWidDefaultValueParameters'];
			Map method = descriptor.toMap()['methods'][0];
			expect(method['name'], 'm1');
